...
export default function AssignmentForm({ form, setForm, attachments, setAttachments, onAttachFile, assignmentId }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      {/* Max score + due date — stack on mobile */}
      <div style={{
        display: "grid",
          ...
          {Object.entries(form.rubric || {}).map(([k, v]) => (
            <div key={k} style={{
              ...
              <span style={{
                ...
                // On mobile, fill available width so progress bar goes below
              ...
              <div style={{
                flex: 1,
                // On mobile, force it onto its own row by taking full width
                ...(isMobile ? { flexBasis: "100%", marginTop: "-6px" } : {}),
                height: "7px",
                background: "#e2e8f0", borderRadius: "4px", overflow: "hidden",
                minWidth: isMobile ? undefined : "60px",
              }}>
                <div style={{
                  height: "7px",
                  background: "linear-gradient(90deg,#3b82f6,#38bdf8)",
                  borderRadius: "4px", width: `${v}%`, transition: "width 0.2s",
                }} />
              </div>
            </div>
          ))}
          <p style={{
            fontSize: "12px", fontWeight: "700", margin: "4px 0 0",
            color: rubricTotal !== 100 ? "#dc2626" : "#16a34a",
          }}>
            Total: {rubricTotal}% {rubricTotal !== 100 && "⚠️ Must equal 100"}
          </p>
        </div>
      ...
      <div>
        ...
        {/* File type buttons — wrap naturally, full width on mobile */}
        <div style={{
          display: "grid",
          ...
          {FILE_TYPES.map(ft => (
            <button
              key={ft.label}
              type="button"
              onClick={() => triggerUpload(ft.accept)}
              style={{
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            {attachments.map((f, i) => (
              <div key={i} style={{
                ...
                // On very small screens, allow chip to stretch
