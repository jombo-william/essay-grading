...
//   // Submissions that have an AI score and can be bulk-approved
...
//       });
//       alert(`✅ ${res.message}`);
//       onGrade(null); // trigger parent refresh
//     } catch (err) {
...
//   return (
//     <div>
//       {/* ── Header row with Approve All button ── */}
...
//             style={{ ...btnGreen, opacity: approvingAll ? 0.6 : 1, cursor: approvingAll ? 'not-allowed' : 'pointer' }}
//           >
...
//         const isHighAI   = (sub.ai_detection_score ?? 0) >= 50;

//         return (
//           <div key={sub.id} className={`pg-card ${isHighAI ? 'pg-card-flagged' : hasAiScore ? 'pg-card-warn' : 'pg-card-neutral'}`}>
//             <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
//               <div style={{ display: 'flex', gap: 12, flex: 1 }}>
//                 <div className={`pg-avatar-lg ${isHighAI ? 'pg-avatar-lg--red' : hasAiScore ? 'pg-avatar-lg--amber' : 'pg-avatar-lg--gray'}`}>
//                   {sub.student_name?.charAt(0)}
//                 </div>
//                 <div>
//                   <p style={{ fontWeight: 800, fontSize: 15, color: 'var(--text)', margin: '0 0 2px' }}>{sub.student_name}</p>
...
//                 <p style={{ fontSize: 11, color: 'var(--text-faint)', margin: '0 0 2px', fontWeight: 600 }}>AI Score</p>
//                 {hasAiScore ? (
//                   <p style={{ fontSize: 20, fontWeight: 800, color: isHighAI ? 'var(--red)' : 'var(--blue)', margin: 0, lineHeight: 1 }}>
//                     {isHighAI ? 0 : sub.ai_score}/{sub.max_score}
//                   </p>
//                 ) : (
//                   <p style={{ fontSize: 13, fontWeight: 700, color: '#94a3b8', margin: 0, lineHeight: 1 }}>
//                     Not yet graded
//                   </p>
//                 )}
//                 {sub.ai_detection_score !== null && (
//                   <div style={{ marginTop: 5 }}>
//                     <span className={`ai-pill ${isHighAI ? 'ai-pill--red' : (sub.ai_detection_score ?? 0) >= 30 ? 'ai-pill--amber' : 'ai-pill--green'}`}>
//                       <span style={{ fontSize: 9 }}>{isHighAI ? '🚨' : (sub.ai_detection_score ?? 0) >= 30 ? '⚠️' : '✅'}</span>
//                       {sub.ai_detection_score}% AI
//                     </span>
//                   </div>
//                 )}
...
//             {/* Show "awaiting GC grading" banner when no AI score yet */}
//             {!hasAiScore && !isHighAI && (
//               <div style={{ marginTop: 10, background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '8px 12px', display: 'flex', gap: 8, alignItems: 'center' }}>
//                 <span>📋</span>
...
//                 <div style={{
//                   width: 40, height: 40, borderRadius: '50%',
//                   background: (gradeModal.ai_detection_score ?? 0) >= 50
//                     ? 'linear-gradient(135deg,#ef4444,#f87171)'
//                     : 'linear-gradient(135deg,#3b82f6,#38bdf8)',
//                   color: '#fff', fontWeight: 800, fontSize: 16,
//                   display: 'flex', alignItems: 'center', justifyContent: 'center',
...
//               </div>
//               <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
//                 {gradeModal.ai_detection_score !== null && (
//                   <span className={`ai-pill ${(gradeModal.ai_detection_score ?? 0) >= 50 ? 'ai-pill--red' : (gradeModal.ai_detection_score ?? 0) >= 30 ? 'ai-pill--amber' : 'ai-pill--green'}`}>
//                     {(gradeModal.ai_detection_score ?? 0) >= 50 ? '🚨' : (gradeModal.ai_detection_score ?? 0) >= 30 ? '⚠️' : '✅'} {gradeModal.ai_detection_score}% AI
//                   </span>
//                 )}
//                 <button onClick={() => setGradeModal(null)} style={{
//                   width: 36, height: 36, borderRadius: 10,
...
//               {/* LEFT — Essay */}
//               <div style={{ flex: 1, overflowY: 'auto', padding: '24px', borderRight: '1px solid #e2e8f0' }}>
//                 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
//                   <p style={{ fontSize: 11, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>
//                     Student Essay
//                   </p>
...
//                       <span>📊 AI: {(gradeModal.ai_detection_score ?? 0) >= 50 ? 0 : gradeModal.ai_score}/{gradeModal.max_score}</span>
//                     )}
//                     {gradeModal.file_name && <span>📎 {gradeModal.file_name}</span>}
//                   </div>
//                 </div>
...
//                 <div style={{
//                   background: '#fffef7', border: '1px solid #e2e8f0', borderRadius: 12,
//                   padding: '28px 32px', fontSize: 14, lineHeight: '1.9', color: '#1e293b',
//                   whiteSpace: 'pre-wrap', minHeight: '400px', fontFamily: 'Georgia, serif',
//                   boxShadow: 'inset 0 1px 4px rgba(0,0,0,0.03)',
//                 }}>
//                   {gradeModal.essay_text || 'No essay text available.'}
//                 </div>
...
//               {/* RIGHT — Grading panel */}
//               <div style={{
//                 width: '320px', flexShrink: 0, overflowY: 'auto', padding: '24px',
...
//                 {/* AI score summary — only if available */}
//                 {gradeModal.ai_score !== null && gradeModal.ai_score !== undefined ? (
//                   <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '14px 16px' }}>
//                     <p style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', margin: '0 0 4px', textTransform: 'uppercase' }}>AI Suggested Score</p>
//                     <p style={{ fontSize: 24, fontWeight: 900, color: (gradeModal.ai_detection_score ?? 0) >= 50 ? '#dc2626' : '#2563eb', margin: 0 }}>
//                       {(gradeModal.ai_detection_score ?? 0) >= 50 ? 0 : gradeModal.ai_score}/{gradeModal.max_score}
//                     </p>
//                     <p style={{ fontSize: 11, color: '#94a3b8', margin: '4px 0 0' }}>
//                       {aiLabel(gradeModal.ai_detection_score ?? 0)} · {gradeModal.ai_detection_score}% AI detected
//                     </p>
//                   </div>
//                 ) : (
...
//                       Grade this submission manually using the score input below, or grade via Google Classroom first.
//                     </p>
//                   </div>
//                 )}
