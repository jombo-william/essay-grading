...
// src/components/teacher/IntegrationsTab.jsx
// No external CSS — all styles inline. Uses Tabler Icons via Icon component.
// Mobile-responsive: all flex rows wrap, paddings scale down, buttons go full-width on small screens.
...
function StepBlock({ step, label, children }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
        <div style={{
          width: 20, height: 20, borderRadius: "50%", background: "#1A1830",
...
function Btn({ onClick, disabled, loading, icon, children, color = "dark", small = false, fullWidth = false, mobileFull = false }) {
  ...
  const p = palettes[color] || palettes.dark;
  const isGhost = color === "ghost";
  const goFull = fullWidth || (mobileFull && isMobile);
      ...
      {results?.map((r, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            border: `1px solid ${r.status === "graded" ? C.green.border : C.red.border}`,
            color: r.status === "graded" ? C.green.text : C.red.text,
            // Stack name + score vertically on very small screens
            display: "flex", alignItems: "flex-start", gap: 6, flexWrap: "wrap",
          }}>
            <Icon
              name={r.status === "graded" ? "circle-check" : "circle-x"}
              size={12}
              style={{ color: r.status === "graded" ? C.green.text : C.red.text, flexShrink: 0, marginTop: 1 }}
            />
            <span style={{ fontWeight: 600, flex: 1, minWidth: 0, wordBreak: "break-word" }}>{displayName}</span>
            {r.status === "graded"
              ? <span style={{ whiteSpace: "nowrap" }}>Score: <strong>{r.score}</strong></span>
              : <span style={{ color: C.red.text, flex: "1 1 100%", paddingLeft: 18 }}>{r.error}</span>
            }
          </div>
        );
      })}
...
function PlatformCard({ icon, iconColor, title, subtitle, children }) {
  ...
  return (
    <div style={{
      ...
      border: "1px solid #ECECF2", marginBottom: 20,
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        <div style={{
          width: isMobile ? 36 : 42, height: isMobile ? 36 : 42,
          borderRadius: 11, background: iconColor,
          display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
        }}>
          <Icon name={icon} size={isMobile ? 17 : 20} style={{ color: "#fff" }} />
        </div>
        <div>
          <p style={{ margin: 0, fontWeight: 700, fontSize: isMobile ? 14 : 15, color: "#1A1830" }}>{title}</p>
          <p style={{ margin: 0, fontSize: 12, color: "#8884A8" }}>{subtitle}</p>
        </div>
      </div>
      {children}
...
// ── Course action button row — wraps gracefully on mobile ─────────────────────
function CourseActions({ children }) {
  return (
    <div style={{
...
export default function IntegrationsTab({ selectedClass, showToast, assignments }) {
  ...
  const syncFromGc = async (courseId) => {
    if (!linkClassId) {
      showToast("Select a local class first, then click Sync", "error");
      return;
    }
    setGcLoading(true);
    try {
  ...
  const inputStyle = {
    ...
    border: `1px solid ${C.gray.border}`, fontSize: 13,
    fontFamily: "inherit", background: "#F8F7FF",
    color: "#1A1830", outline: "none", boxSizing: "border-box",
  };

  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", maxWidth: "100%" }}>
      <style>{`
        @keyframes spin { to { transform: rotate(360deg); } }
      `}</style>

      <p style={{ fontSize: isMobile ? 15 : 17, fontWeight: 700, color: "#1A1830", margin: "0 0 20px" }}>
        External integrations
      </p>
      ...
      <PlatformCard icon="brand-google" iconColor="#185FA5" title="Google Classroom" subtitle="Import and auto-grade student submissions">
        ...
        {/* Step 1 */}
        <StepBlock step={1} label="Connect your Google account">
          <Btn icon="lock" color="blue" mobileFull onClick={connectGoogle}>
        ...
        {/* Step 2 */}
        <StepBlock step={2} label="Load your courses">
          <Btn icon="books" color="purple" loading={gcLoading} mobileFull onClick={loadGcCourses}>
            {gcLoading ? "Loading…" : "Load my courses"}
          </Btn>
        </StepBlock>
            ...
            <StepBlock step={3} label="Link each course to a local class">
              {gcCourses.map(course => (
                <div key={course.id} style={{
                  ...
                  borderRadius: 10, marginBottom: 10, background: "#F8F7FF",
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
                    <Icon name="school" size={14} style={{ color: C.blue.text, flexShrink: 0 }} />
                    <p style={{ margin: 0, fontWeight: 700, fontSize: 13, color: "#1A1830", wordBreak: "break-word" }}>
                      {course.name} {course.section}
                    </p>
                  </div>

                  {/* Select + buttons — stack on mobile */}
                  ...
                  <CourseActions>
                    <Btn icon="link" color="green" small loading={linking}
                      onClick={() => linkCourseToClass(course.id)}
                      fullWidth={isMobile}>
                      Link
                    </Btn>
                    <Btn icon="list" color="purple" small loading={gcLoading}
                      onClick={() => loadGcAssignments(course.id)}
                      fullWidth={isMobile}>
                      Assignments
                    </Btn>
                    <Btn icon="refresh" color="amber" small loading={gcLoading}
      ...
      <PlatformCard icon="school" iconColor="#854F0B" title="Moodle" subtitle="Import and auto-grade from your Moodle LMS">
        ...
        <StepBlock step={2} label="Moodle API token">
          ...
          />
          {/* Connect + Open Moodle — stack on mobile */}
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Btn icon="plug-connected" color="amber" loading={moodleLoading}
              onClick={connectMoodle} fullWidth={isMobile}>
              {moodleLoading ? "Connecting…" : "Connect to Moodle"}
            </Btn>
            <a
              ...
              style={{
                ...
                background: C.gray.bg, color: C.gray.dark,
                border: `1px solid ${C.gray.border}`,
                fontWeight: 500, fontSize: 13, textDecoration: "none", whiteSpace: "nowrap",
        ...
        {moodleConnected && (
          <div style={{
            ...
            marginBottom: 16, maxWidth: "100%", flexWrap: "wrap",
            wordBreak: "break-all",
          }}>
            <Icon name="circle-check" size={13} style={{ color: C.green.text, flexShrink: 0 }} />
            Connected to {moodleSiteUrl}
          </div>
        )}
          ...
          <>
            ...
            <StepBlock step={3} label="Select your Moodle course">
              {moodleCourses.map(course => (
                <div key={course.id} style={{
                  padding: isMobile ? 12 : 14,
                  border: `1px solid ${selectedMoodleCourse == course.id ? C.amber.border : C.gray.border}`,
                  borderRadius: 10, marginBottom: 10,
                  background: selectedMoodleCourse == course.id ? C.amber.bg : "#F8F7FF",
                }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 7, marginBottom: 10 }}>
                    <Icon name="books" size={14} style={{ color: C.amber.text, flexShrink: 0, marginTop: 2 }} />
                    <p style={{ margin: 0, fontWeight: 700, fontSize: 13, color: "#1A1830", wordBreak: "break-word" }}>
                      {course.fullname}
                    </p>
                  </div>
                  <Btn icon="list" color="amber" small loading={moodleLoading}
