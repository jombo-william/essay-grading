...
//     try {
//       const res = await apiFetch(`/moodle/courses?moodle_token=${encodeURIComponent(moodleToken)}`);
...
//     setMoodleLoading(true);
//     try {
//       const res = await apiFetch(`/moodle/assignments?moodle_token=${encodeURIComponent(moodleToken)}&course_id=${courseId}`);
...
//         method: "POST",
//         body: JSON.stringify({ moodle_token: moodleToken, moodle_assignment_id: parseInt(selectedMoodleAssign), local_assignment_id: parseInt(moodleLocalId) }),
//       });
...
export default function IntegrationsTab({ selectedClass, showToast, assignments }) {
  ...
  const connectMoodle = async () => {
    ...
    setMoodleLoading(true);
    try {
      // FIXED: Added site_url parameter
      const res = await apiFetch(`/moodle/courses?moodle_token=${encodeURIComponent(moodleToken)}&site_url=${encodeURIComponent(moodleSiteUrl)}`);
      setMoodleCourses(res.courses || []);
      setMoodleConnected(true);
      showToast(`Connected — found ${res.courses.length} courses`, "success");
    } catch (err) {
  ...
  const loadMoodleAssignments = async (courseId) => {
    ...
    try {
      // FIXED: Added site_url parameter
      const res = await apiFetch(`/moodle/assignments?moodle_token=${encodeURIComponent(moodleToken)}&course_id=${courseId}&site_url=${encodeURIComponent(moodleSiteUrl)}`);
  ...
  const gradeFromMoodle = async () => {
    ...
    setMoodleResults(null);
    try {
      // FIXED: Added site_url to POST body
      const res = await apiFetch("/moodle/autograde", {
        method: "POST",
        body: JSON.stringify({ 
          moodle_token: moodleToken, 
