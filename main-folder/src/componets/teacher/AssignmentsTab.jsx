...
const css = `
  ...
  .at-btn-lbl { display: inline; }

  /* Modal body scroll */
  .at-modal-body { flex: 1; overflow-y: auto; padding: 24px 28px; }
  ...
  @media (max-width: 600px) {
    .at-card { padding: 13px 13px; }
    .at-card-row { flex-direction: column; gap: 10px; }
    .at-actions { width: 100%; justify-content: flex-end; }
    .at-btn { padding: 6px 9px; font-size: 11px; }
    .at-btn-lbl { display: none; }

    .at-modal-body { padding: 16px 16px; }

    .at-detail-body { flex-direction: column; }
    .at-detail-left { border-right: none; border-bottom: 1px solid #ECECF2; padding: 16px; }
    .at-detail-right { width: 100%; padding: 16px; }

    .at-header-row { flex-direction: column; align-items: flex-start; }
  }
`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 15, fontWeight: 700, color: "#1A1830", margin: "0 0 8px" }}>{title}</p>
        <p style={{ fontSize: 13, color: "#8884A8", lineHeight: 1.6, margin: "0 0 22px" }}>{message}</p>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{
            padding: "8px 16px", borderRadius: 9,
            border: "1px solid #D3D1C7", background: "#F1EFE8",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          display: "flex", alignItems: "center", justifyContent: "space-between",
          padding: "13px 18px", borderBottom: "1px solid #ECECF2",
          background: "#F8F7FF", flexShrink: 0, gap: 10,
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontWeight: 700, fontSize: 14, color: "#1A1830", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {assignment.title}
              </p>
              <p style={{ fontSize: 11, color: "#8884A8", margin: 0 }}>
                {isArchived ? "Archived" : isPast ? "Closed" : "Active"} · {assignment.max_score} pts ·{" "}
                Due {assignment.due_date
                  ? new Date(assignment.due_date.replace(" ", "T")).toLocaleDateString("en-GB", {
                      timeZone: "Africa/Blantyre", day: "numeric", month: "short", year: "numeric",
        ...
        <div className="at-detail-body">
          <div className="at-detail-left">
            <p style={{ fontSize: 10, fontWeight: 700, color: "#B0AECB", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 12px" }}>
              Essay instructions
            </p>
            {assignment.description && (
              <p style={{ fontSize: 13, color: "#8884A8", margin: "0 0 12px", lineHeight: 1.6 }}>
                {assignment.description}
              </p>
            )}
            <div style={{
              background: "#FDFCF7", border: "1px solid #ECECF2", borderRadius: 12,
              padding: "20px 22px", fontSize: 14, lineHeight: 1.9, color: "#1A1830",
              whiteSpace: "pre-wrap", minHeight: 200, fontFamily: "Georgia, serif",
              marginBottom: 16, boxSizing: "border-box",
            }}>
              {assignment.instructions}
            </div>
            {assignment.reference_material && (
              <details style={{ background: "#E6F1FB", border: "1px solid #B5D4F4", borderRadius: 11, padding: "10px 13px" }}>
                <summary style={{ fontSize: 12, fontWeight: 600, color: "#185FA5", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                  <Icon name="robot" size={13} style={{ color: "#185FA5" }} />
                  View AI reference material
                </summary>
                <p style={{ fontSize: 13, color: "#1A1830", lineHeight: 1.7, margin: "8px 0 0", whiteSpace: "pre-wrap" }}>
                  {assignment.reference_material}
                </p>
              </details>
            )}
          ...
          <div className="at-detail-right">
            ...
            {stats.map(s => (
              <div key={s.label} style={{
                ...
                <div>
                  <p style={{ fontSize: 19, fontWeight: 700, color: "#1A1830", margin: 0, lineHeight: 1 }}>{s.value}</p>
                  <p style={{ fontSize: 11, color: "#8884A8", margin: "2px 0 0", fontWeight: 500 }}>{s.label}</p>
                </div>
              </div>
            ))}
            {assignment.rubric && (
              <div style={{ background: "#fff", border: "1px solid #ECECF2", borderRadius: 11, padding: "12px 14px" }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: "#B0AECB", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 10px" }}>Rubric</p>
                {Object.entries(assignment.rubric).map(([k, v]) => (
                  <div key={k} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 7 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: "#5F5E5A", textTransform: "capitalize", width: 60, flexShrink: 0 }}>{k}</span>
                    <div style={{ flex: 1, height: 5, background: "#F1EFE8", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: 3, width: `${v}%`, background: "#1A3A6B" }} />
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 700, color: "#185FA5", width: 28, textAlign: "right" }}>{v}%</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{
          ...
          gap: 8, padding: "11px 18px", borderTop: "1px solid #ECECF2",
          background: "#F8F7FF", flexShrink: 0, flexWrap: "wrap",
        }}>
          <button onClick={() => onExport(assignment)} style={{
            padding: "7px 14px", borderRadius: 9,
            border: "1px solid #B5D4F4", background: "#E6F1FB",
            color: "#185FA5", fontSize: 12, fontWeight: 500,
            cursor: "pointer", fontFamily: "inherit",
            display: "flex", alignItems: "center", gap: 5,
          }}>
            <Icon name="file-export" size={13} style={{ color: "#185FA5" }} />
            Export PDF
          </button>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: iconBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Icon name={iconName} size={17} style={{ color: iconColor }} />
            </div>
            <div>
              <p style={{ fontWeight: 700, fontSize: 14, color: "#1A1830", margin: 0 }}>{title}</p>
              {subtitle && <p style={{ fontSize: 12, color: "#8884A8", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "50vw" }}>{subtitle}</p>}
            </div>
          </div>
          <button onClick={onClose} style={{
            width: 30, height: 30, borderRadius: 8, border: "1px solid #ECECF2",
        ...
        <div style={{
          display: "flex", justifyContent: "flex-end", gap: 8,
          padding: "11px 18px", borderTop: "1px solid #ECECF2",
          background: "#F8F7FF", flexShrink: 0,
        }}>
          <button onClick={onClose} style={{
            padding: "9px 18px", borderRadius: 9, border: "1px solid #D3D1C7",
            background: "#F1EFE8", color: "#5F5E5A", fontSize: 13, fontWeight: 500,
            cursor: "pointer", fontFamily: "inherit",
          }}>Cancel</button>
          <button onClick={onSave} disabled={saving} style={{
...
export default function AssignmentsTab({ assignments, submissions, loading, onCreated, onUpdated, showToast, selectedClassId, selectedClass, archivedOnly = false }) {
  ...
  const renderAssignment = (a) => {
    ...
    return (
      <div key={a.id} className="at-card" style={{
        borderLeft: `4px solid ${leftColor}`,
        opacity: isArchived ? 0.8 : 1,
        background: isArchived ? "#FAFAF8" : "#fff",
      }}>
        <div className="at-card-row">
          {/* Left: info */}
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", marginBottom: 5 }}>
              ...
              <span style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "2px 7px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: "#E6F1FB", color: "#185FA5", border: "1px solid #B5D4F4" }}>
                {a.max_score} pts
              </span>
              <span style={{
                display: "inline-flex", alignItems: "center", gap: 3,
                ...
                <Icon name={isArchived ? "archive" : isPast ? "lock" : "circle-check"} size={10}
                  style={{ color: isArchived ? "#5F5E5A" : isPast ? "#5F5E5A" : "#3B6D11" }} />
                {isArchived ? "Archived" : isPast ? "Closed" : "Active"}
              </span>
              {hasRef && (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "2px 7px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: "#EEEDFE", color: "#3C3489", border: "1px solid #CECBF6" }}>
                  <Icon name="robot" size={10} style={{ color: "#3C3489" }} /> AI ref
                </span>
              )}
            </div>

            {a.description && (
              <p style={{ fontSize: 12, color: "#8884A8", margin: "0 0 7px", lineHeight: 1.5 }}>{a.description}</p>
            )}

            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              <span style={{ fontSize: 11, color: "#B0AECB", display: "flex", alignItems: "center", gap: 4 }}>
                <Icon name="calendar" size={11} style={{ color: "#B0AECB" }} />
                Due {a.due_date ? new Date(a.due_date.replace(" ", "T")).toLocaleDateString("en-GB", { timeZone: "Africa/Blantyre", day: "numeric", month: "short", year: "numeric" }) : "No date"}
              </span>
              <span style={{ fontSize: 11, color: "#B0AECB", display: "flex", alignItems: "center", gap: 4 }}>
                <Icon name="file-text" size={11} style={{ color: "#B0AECB" }} />
                {subCount} submitted
          ...
          <div className="at-actions">
            {!isArchived && (
              <button className="at-btn" onClick={() => openEdit(a)}
                style={{ border: "1px solid #D3D1C7", background: "#F1EFE8", color: "#5F5E5A" }}>
                <Icon name="pencil" size={12} style={{ color: "#5F5E5A" }} />
                <span className="at-btn-lbl">Edit</span>
              </button>
            )}
            <button className="at-btn" onClick={() => setArchiveTarget(a)}
              style={{
                border: `1px solid ${isArchived ? "#C0DD97" : "#D3D1C7"}`,
                background: isArchived ? "#EAF3DE" : "#F1EFE8",
                color: isArchived ? "#3B6D11" : "#5F5E5A",
              }}>
              <Icon name={isArchived ? "arrow-back-up" : "archive"} size={12}
                style={{ color: isArchived ? "#3B6D11" : "#5F5E5A" }} />
              <span className="at-btn-lbl">{isArchived ? "Restore" : "Archive"}</span>
            </button>
            <button className="at-btn" onClick={() => setDeleteTarget(a)}
              style={{ border: "1px solid #F7C1C1", background: "#FCEBEB", color: "#A32D2D" }}>
              <Icon name="trash" size={12} style={{ color: "#A32D2D" }} />
              <span className="at-btn-lbl">Delete</span>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {!archivedOnly && archivedAssignments.length > 0 && (
            <button onClick={() => setShowArchived(v => !v)} style={{
              padding: "7px 13px", borderRadius: 9, border: "1px solid #D3D1C7",
              background: "#F1EFE8", color: "#5F5E5A", fontSize: 12, fontWeight: 500,
              cursor: "pointer", fontFamily: "inherit",
              display: "flex", alignItems: "center", gap: 5,
            }}>
              <Icon name="archive" size={13} style={{ color: "#5F5E5A" }} />
              {showArchived ? "Hide" : `Archived (${archivedAssignments.length})`}
            </button>
          )}
          {!archivedOnly && (
      ...
      {/* Empty states */}
      {activeAssignments.length === 0 && !showArchived && !archivedOnly && (
        <div style={{ background: "#fff", borderRadius: 14, border: "1.5px dashed #D3D1C7", textAlign: "center", padding: "50px 20px" }}>
          <div style={{ width: 46, height: 46, borderRadius: 13, background: "#E6F1FB", margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Icon name="clipboard-list" size={21} style={{ color: "#185FA5" }} />
          </div>
          <p style={{ fontWeight: 700, color: "#1A1830", fontSize: 15, margin: "0 0 4px" }}>No assignments yet</p>
          <p style={{ fontSize: 13, color: "#8884A8", margin: 0 }}>Create your first assignment to get started.</p>
        </div>
      )}
      ...
      {archivedOnly && assignments.length === 0 && (
        <div style={{ background: "#fff", borderRadius: 14, border: "1.5px dashed #D3D1C7", textAlign: "center", padding: "50px 20px" }}>
          <div style={{ width: 46, height: 46, borderRadius: 13, background: "#F1EFE8", margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Icon name="archive" size={21} style={{ color: "#5F5E5A" }} />
          </div>
          <p style={{ fontWeight: 700, color: "#1A1830", fontSize: 15, margin: 0 }}>No archived assignments</p>
        </div>
      )}

      {archivedOnly ? assignments.map(renderAssignment) : activeAssignments.map(renderAssignment)}

      {showArchived && !archivedOnly && archivedAssignments.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: "#B0AECB", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 10px", display: "flex", alignItems: "center", gap: 5 }}>
            <Icon name="archive" size={12} style={{ color: "#B0AECB" }} />
            Archived assignments
          </p>
          {archivedAssignments.map(renderAssignment)}
      ...
      {createOpen && (
        <ModalShell title="New assignment" subtitle="Fill in the details and publish"
          iconName="clipboard-list" iconBg="#E6F1FB" iconColor="#185FA5"
          onClose={() => setCreateOpen(false)} saving={saving}
          onSave={handleCreate} saveLabel="Publish assignment">
          <AssignmentForm form={form} setForm={setForm}
            attachments={attachments} setAttachments={setAttachments}
            onAttachFile={handleAttachFile} assignmentId={null} />
        </ModalShell>
      )}
      ...
      {editTarget && (
        <ModalShell title="Edit assignment" subtitle={editTarget?.title}
          iconName="pencil" iconBg="#FAEEDA" iconColor="#854F0B"
      ...
      {deleteTarget && (
        <ConfirmModal
          title="Delete assignment?"
          message={deleteTarget.gc_coursework_id
            ? `"${deleteTarget.title}" will be permanently deleted from EssayGrade and Google Classroom. This cannot be undone.`
            : `"${deleteTarget.title}" and all its submissions will be permanently deleted. This cannot be undone.`}
          confirmLabel={
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {archiveTarget && (
        <ConfirmModal
          title={archiveTarget.is_active === false ? "Restore assignment?" : "Archive assignment?"}
          message={archiveTarget.is_active === false
            ? `"${archiveTarget.title}" will be made active again and visible to students.`
            : `"${archiveTarget.title}" will be hidden from students. Submissions are kept.`}
          confirmLabel={
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
