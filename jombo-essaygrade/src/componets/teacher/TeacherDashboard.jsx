...
// One colour per class (cycles if > 6 classes)
...
export default function TeacherDashboard({ user, selectedClass, classIndex = 0, onBack, onChangeClass }) {
  ...
  // Fetch everything scoped to the selected class
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const classParam = `?class_id=${selectedClass.id}`;
      const [aData, sData] = await Promise.all([
        apiFetch(`/api/teacher/assignments${classParam}`),
        apiFetch(`/api/teacher/submissions${classParam}`),
      ]);
      setAssignments(aData.assignments || []);
      setSubmissions(sData.submissions || []);
    } catch (err) {
  ...
  // Derived data
  const pending = submissions.filter(s => s.status !== "graded" && !s.final_score);
  const students = Array.from(
    submissions.reduce((map, s) => {
  ...
  const saveGrade = async () => {
    if (!gradeScore) { showToast("Please enter a score.", "error"); return; }
    try {
      await apiFetch("/api/teacher/submissions/grade", {
        method: "POST",
        body: JSON.stringify({ submission_id: gradeSub.id, score: gradeScore, feedback: gradeFB }),
      });
      setGradeSub(null);
      ...
      <div style={{ maxWidth: "880px", margin: "0 auto", padding: "28px 20px" }}>
        ...
        {tab === "students" && (
          <StudentsTab
            ...
            assignments={assignments} loading={loading}
            selectedClass={selectedClass}
          />
        )}
      </div>
