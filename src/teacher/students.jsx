...
const C = {
  ...
  badge: c => ({ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', background: c === 'green' ? '#f0fdf4' : c === 'red' ? '#fef2f2' : c === 'amber' ? '#fffbeb' : '#f1f5f9', color: c === 'green' ? '#16a34a' : c === 'red' ? '#dc2626' : c === 'amber' ? '#d97706' : '#64748b' }),
                ...
                {MOCK_STUDENTS.map(student => {
                  ...
                  return (
                    <tr key={student.id} style={{ borderBottom: '1px solid #f1f5f9' }}
                      ...
                      <td style={{ padding: '8px 10px' }}>
                        {flagged > 0 ? <span style={C.badge('red')}>🚨 {flagged}</span> : <span style={{ fontSize: '12px', color: '#16a34a' }}>✅ None</span>}
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        {avg !== null ? <span style={{ fontWeight: '800', color: scoreColor(avg), fontSize: '13px' }}>{avg}%</span> : <span style={{ color: '#94a3b8' }}>—</span>}
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        ...
                        {subs.map(sub => (
                          <tr key={sub.id} style={{ borderBottom: '1px solid #f1f5f9' }}
                            ...
                            <td style={{ padding: '8px 10px' }}>
                              ...
                              {sub.status === 'pending' && <span style={C.badge(null)}>🤖 Grading</span>}
                            </td>
                            <td style={{ padding: '8px 10px' }}>
                              <div style={{ display: 'flex', gap: '4px' }}>
                                <button onClick={() => setSubmissionDetail(sub)} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontSize: '10px', fontWeight: '600', cursor: 'pointer' }}>View</button>
                                {sub.status === 'ai_graded' && <button onClick={() => { setStudentTableModal(null); openGrade(sub); }} style={{ padding: '4px 8px', borderRadius: '6px', border: 'none', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: '#fff', fontSize: '10px', fontWeight: '700', cursor: 'pointer' }}>Grade</button>}
                                {sub.status === 'graded' && <button onClick={() => { setStudentTableModal(null); openEditGrade(sub); }} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid #c7d2fe', background: '#eff6ff', color: '#4f46e5', fontSize: '10px', fontWeight: '700', cursor: 'pointer' }}>✏️ Edit</button>}
                              </div>
                            </td>
                          </tr>
                        ))}
