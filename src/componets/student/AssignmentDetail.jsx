  ...
  const footer = !a.submitted && !a.isPast ? (
    ...
    <div style={{ display: 'flex', gap: '10px' }}>
      <button
        ...
        style={{ ...C.gBtn, border: '1.5px solid #1a2e5a', background: 'transparent', color: '#1a2e5a' }}
      >
        View My Essay
      </button>
    ...
    <p style={{ textAlign: 'center', color: '#1a2e5a', fontSize: '13px', margin: 0 }}>
    ...
    >
      ...
      {a.submitted && (
        <div style={{
          ...
          <div>
            <p style={{ fontWeight: '700', color: '#1a2e5a', fontSize: '13px', margin: 0 }}>
              You have submitted this assignment
            </p>
            <p style={{ fontSize: '12px', color: '#1a2e5a', margin: 0 }}>
              {a.submission && a.submission.final_score !== null
      ...
      {a.isPast && !a.submitted && (
        <div style={{
          ...
          <span style={{ fontSize: '20px' }}>⏰</span>
          <p style={{ fontWeight: '700', color: '#dc2626', fontSize: '13px', margin: 0 }}>
      ...
      <div style={{ marginBottom: '18px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#1a2e5a', lineHeight: 1.7, margin: 0 }}>{a.description}</p>
      </div>
      ...
      <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#1a2e5a', margin: 0, lineHeight: 1.7 }}>{a.instructions}</p>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {Object.entries(a.rubric).map(([k, v]) => (
              <div key={k} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '13px', color: '#1a2e5a', textTransform: 'capitalize', fontWeight: '600', width: '100px', flexShrink: 0 }}>{k}</span>
                <div style={{ flex: 1, height: '8px', background: '#f1f5f9', borderRadius: '4px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${v}%`, background: 'linear-gradient(90deg,#6366f1,#8b5cf6)', borderRadius: '4px' }} />
                </div>
                <span style={{ fontSize: '13px', color: '#1a2e5a', fontWeight: '800', width: '36px', textAlign: 'right' }}>{v}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        {[
          ...
          <div key={d.label} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px 12px' }}>
            <p style={{ fontSize: '11px', color: '#1a2e5a', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 3px' }}>{d.label}</p>
            <p style={{ fontSize: '13px', color: '#1a2e5a', fontWeight: '600', margin: 0 }}>{d.icon} {d.value}</p>
          </div>
        ))}
      </div>
    </Sheet>
  ...
  return (
    <div style={{ background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        <p style={{ fontSize: '12px', fontWeight: '800', color: '#1a2e5a', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>
          📚 Reference Material
        </p>
        <span style={{ fontSize: '11px', color: '#1a2e5a', background: '#e0f2fe', padding: '2px 8px', borderRadius: '20px', fontWeight: '700' }}>
          From Teacher
        </span>
      </div>
      <p style={{ fontSize: '12px', color: '#1a2e5a', margin: '0 0 10px', fontStyle: 'italic' }}>
