...
export default function AssignmentsTab({
  ...
  if (loading) {
    return (
      <div style={{ ...C.card, textAlign: 'center', padding: '48px 24px' }}>
        ...
        />
        <p style={{ color: '#ffffff', fontSize: '14px', margin: 0 }}>Loading assignments…</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <p style={{ fontSize: '18px', fontWeight: '800', color: '#ffffff', margin: '0 0 16px' }}>
        Your Assignments
      </p>
      ...
      {assignments.length === 0 && (
        <div style={{ ...C.card, textAlign: 'center', padding: '48px 24px' }}>
          ...
          <p style={{ fontWeight: '700', color: '#ffffff', fontSize: '14px', margin: 0 }}>
      ...
      {assignments.map(a => (
        <div
          ...
          style={{
            ...C.card,
            borderLeft: `4px solid ${a.submitted ? '#8b5cf6' : a.isPast ? '#ef4444' : '#6366f1'}`,
            cursor: 'pointer',
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: '13px', color: '#ffffff', margin: '0 0 8px', lineHeight: 1.5 }}>{a.description}</p>
              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '12px', color: '#ffffff' }}>
                  📅 Due {new Date(a.due_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                </span>
                <span style={{ fontSize: '12px', color: '#ffffff' }}>🏆 {a.max_score} pts</span>
              </div>
            </div>
            ...
            return (
              ...
              >
                  ...
                  <div>
                    <p style={{ fontSize: '12px', fontWeight: '700', color: isAI ? '#dc2626' : '#6d28d9', margin: 0 }}>
                      {sub.final_score !== null
                        ...
                        : isAI
                          ? `AI Flagged (${sub.ai_detection_score}%) — Score: 0`
                          : sub.status === 'pending'
                            ? 'Grading in progress...'
                            : 'AI graded — awaiting teacher'}
                    </p>
                    <p style={{ fontSize: '11px', color: '#1a2e5a', margin: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  {sub.final_score !== null && (
                    <span style={{ fontSize: '12px', color: '#1a2e5a', fontWeight: '700' }}>See feedback →</span>
                  )}
                </div>
              </div>
            );
