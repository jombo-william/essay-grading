...
export default function StudentDashboard({ onBack }) {
  ...
  const handleFileChange = e => {
    ...
    if (file.type === 'text/plain') {
      ...
      const r = new FileReader();
      r.onload = () => {
        const matches = (r.result.match(/\((.*?)\)/g) || []);
        let txt = matches.map(m => m.slice(1,-1).replace(/\\n/g,' ').replace(/\\/g,'')).filter(t => t.trim().length > 2 && /[a-zA-Z]/.test(t)).join(' ');
        if (txt.trim().split(/\s+/).length < 20) txt = `[File: "${file.name}"]\n\nContent extracted for AI grading.`;
        setUploadText(txt);
      };
      r.readAsBinaryString(file);
    } else {
      setUploadText(`[File attached: "${file.name}" — ${(file.size/1024).toFixed(1)} KB]\n\nThis file will be processed for grading.`);
    }
  };

  const openWrite = a => { setWriteModal(a); setSubmitMode('write'); setEssayText(''); setUploadFile(null); setUploadText(''); };
          ...
          footer={
            !assignmentModal.submitted && !assignmentModal.isPast ? (
              ...
              <div style={{ display:'flex', gap:'10px' }}>
                <button onClick={() => { const sub = assignmentModal.submission; if(sub) { setAssignmentModal(null); setEssayViewModal(sub); }}}
                  style={{ ...C.gBtn, border:'1.5px solid #8b5cf6', background:'transparent', color:'#8b5cf6' }}>View My Essay</button>
                {assignmentModal.submission?.final_score !== null && (
                  <button onClick={() => { setAssignmentModal(null); setResultModal(assignmentModal.submission); }}
                    style={C.pBtn(false)}>See Results →</button>
                )}
              </div>
            ) : (
              ...
              <div>
                ...
                <p style={{ fontSize:'12px', color:'#94a3b8', margin:0 }}>
                  {assignmentModal.submission?.final_score !== null
                    ...
                    : assignmentModal.submission?.ai_score !== null ? 'Awaiting teacher approval' : 'Grading in progress'}
                </p>
              </div>
            </div>
          )}
